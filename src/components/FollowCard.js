    ...
    const followUnfollow = async () => {
        ...
        try {
            const { data } = await axios.post(
                `https://missingdata.pythonanywhere.com/${isFollow ? 'unfollow' : 'follow'
                }`,
                { user_id: id },
                {
                    headers: {
            ...
            if (data) {
                ...
                {
                    !isFollowing ? dispatch(updateFollowFollowingList()) : null;
                }
            }
    ...
    return (
        <View>
            <View style={[styles.container, { backgroundColor }]}>
                <Image
                    style={styles.avatar}
                    source={{ uri: 'https://via.placeholder.com/50x50.png?text=DP' }}
                />
                <View style={styles.textContainer}>
                    <Text
                        numberOfLines={1}
                        style={[styles.name, { color: textColor, maxWidth: 180 }]}>
                        {username}
                    </Text>
                    <Text
                        numberOfLines={1}
                        style={[styles.username, { color: textColor, maxWidth: 180 }]}>
                        {email}
                    </Text>
                </View>
